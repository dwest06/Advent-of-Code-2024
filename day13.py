...
def read_puzzles(puzzles):
    ...
    for puzzle in puzzles:
        ...
        for line in puzzle:
            if "Button A" in line:
                button = line.split(":")[1].split(",") # Button A and X Y
                buttons["A"] = {"X": int(re.findall("\d+", button[0])[0]), "Y": int(re.findall("\d+", button[1])[0])}
            elif "Button B" in line:
                button = line.split(":")[1].split(",") # Button B and X Y
                buttons["B"] = {"X": int(re.findall("\d+", button[0])[0]), "Y": int(re.findall("\d+", button[1])[0])}
            else:
                line = line.split(":")[1].split(", ")
                buttons["Prize"] = {"X": int(re.findall("\d+", line[0])[0]), "Y": int(re.findall("\d+", line[1])[0])}
        p.append(buttons)
...
def calculate(puzzle, x, y, a, b):

    if x in DP and y in DP[x] and a in DP[x][y] and b in DP[x][y][a]:
        return DP[x][y][a][b]

    if puzzle['Prize']["X"] == x and puzzle['Prize']["Y"] == y:
        return (a, b)
    
    if puzzle['Prize']["X"] < x or puzzle['Prize']["Y"] < y:
        return False
    
    # if a > 100 or b > 100:
    #     return False
    
    result = calculate(puzzle, x + puzzle["A"]["X"], y + puzzle["A"]["Y"], a + 1, b) or \
        calculate(puzzle, x + puzzle["B"]["X"], y + puzzle["B"]["Y"], a, b + 1)
...
def main(puzzles):
    ...
    wins = []
    for puzzle in puzzles:
        # Add 10000000000000 to prize positions PART 2
        puzzle["Prize"]["X"] += 10000000000000
        puzzle["Prize"]["Y"] += 10000000000000
        DP = {}
        result = calculate(puzzle, 0, 0, 0, 0)
        if result:
            wins.append(result)
...
print(main(read_puzzles(puzzles)))